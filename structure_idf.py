...
def struct_identifier(text):
    doc = nlp(text)  # Process the text
    structure_counts = {}  # Dictionary to store structure counts

# If the input is node  # If the input is node (an instance of Tree, instead of a string)
# def struct_idf(doc):
#     structure_counts = {} 

    for sent in doc.sents:
        parse_string = sent._.parse_string  # Get the const parsing as a string in Penn Treebank format
        tree = Tree.fromstring(parse_string)  # Convert the string into tree object to manipulate
        tree.pretty_print()  # Visualizes the parse tree
        print(parse_string)

        # Function for updating
        def add_structure_counts(struct_name):
            # Adding the struct_name to structure_counts and count the occurances
            structure_counts[struct_name] = structure_counts.get(struct_name, 0) + 1

        # WH QUESTIONS WITH AUX_INV 'Did you sleep well?'
        def wh_q_aux_inv(node):  # Accept input 'node'
            ...
            if node.label() == 'SBARQ':  # Traverse the tree looking for a node labeled 'SBARQ'
                ...
                for child in node:  # For each child of SBARQ
                    if isinstance(child, Tree):  # Make sure if the child is a node not the words which is string
                ...
                if has_wh and sq_node:
                    ...
                    for word in sq_node.leaves():  # .leaves() returns all terminal (leaf) words from the tree
                        spacy_token = next((token for token in sent if token.text == word),
                                           None)  # Retrieve the first word of sq_node leaves next(x,y)
        ...
        def yn_q_no_aux_inv(node):
            ...
            if node.label() == 'S':  # Check if the current node is 'S'
                ...
                if found_vp:  # If there's VP after NP under S
                    ...
                    if '?' in node.leaves():  # '?' in terminal symbols
                ...
                for child in node:  # Iterate through the children of S
                    if isinstance(child, Tree):  # Ensure the input is an instance of the class Tree
                        ...
                        if child.label() == 'NP':  # If NP is found
                            ...
                            if any(isinstance(grandchild, Tree) and grandchild.label() == 'TO' for grandchild in
                                   child):  # Check if there is a node (an instance of Tree) under VP labeled 'TO'
                                return False  # Return False
            ...
            for child in node:  # for each child of any node
                if isinstance(child, Tree):  # Ensure the input is an instance of the class Tree
                    if child.label() in {'NP', 'NN'}:  # if found NP or NN
                        ...
                        for grandchild in child:  # for each grandchild ('grandchild' means the immediate child of VP)
                            if isinstance(grandchild,
                                          Tree) and grandchild.label() == 'MD':  # if a Tree instance labeled MD found under VP
                                found_md = True  # Identify the presence of MD under VP
                            elif isinstance(grandchild,
                                            Tree) and grandchild.label() == 'VP' and found_md:  # if found VP after found MD under VP
            ...
            for child in node:  # For any child under S
                if isinstance(child, Tree):  # Ensure the input is an instance of the class Tree
                    if child.label() == 'NP':  # if found NP
                        ...
                        if not any(isinstance(grandchild, Tree) and grandchild.label() == 'MD' for grandchild in
                                   child):  # Check if no nodes under VP labeled MD ('not' negates both isinstance and .label())
        ...
        def fin_neg(node):
            ...
            for child in node:  # For each child of any node
                if isinstance(child, Tree):  # Ensure the node is an instance of the class Tree
                    ...
                    elif child.label() == 'RB' and found_verb and has_neg:  # If found RB, verbs mentioned in the same depth, and a neg word in the leaves (but i think the order of found_verb and RB is ignored here)
            ...
            if node.label() == 'S':  # if found S
                for child in node:  # for each child of the node S
                    if isinstance(child, Tree) and child.label() == 'VP':  # if it is a tree instance and labeled VP
                        ...
                        for grandchild in child:  # for each grandchild ('grandchild' refers to the immediate child of VP)
                            if isinstance(grandchild, Tree) and grandchild.label() == 'TO':  # if found TO under VP
                                has_to = True  # Assign node TO under VP to has_to
                            elif isinstance(grandchild,
                                            Tree) and grandchild.label() == 'VP' and has_to:  # if found VP (under VP) and node has_to is True (meaning it's been encountered before)
        ...
        def prep_adv(node):
            ...
            for child in node:  # For any child in the current node (the label for root node does not matter)
                if isinstance(child, Tree):  # Ensure the node is a tree instance
                    ...
                    elif child.label() == 'NP' and (
                            found_rb or found_pp):  # check if NP found after found_rb or found_pp True (there's NP after adverb/pp)
        ...
        def rc(node):
            ...
            if node.label() == 'NP':  # Check if the current node is labeled 'NP' as relative clauses should occur under NP
                has_np_in_np = any(child.label() == 'NP' for child in node if isinstance(child,
                                                                                         Tree))  # If found NP in the children of node NP assign it into has_np_in_np
                if has_np_in_np:  # if has_np_in np True
                    for child in node:  # for each child of the node  of first NP
                        if isinstance(child,
                                      Tree) and child.label() == 'SBAR':  # If node SBAR found after has_np_in_np True (indicating there is NP and SBAR in the same depth level)
                            ...
                            for sbar_child in child:  # For each child of node SBAR
                                if isinstance(sbar_child,
                                              Tree) and sbar_child.label() == 'WHNP':  # if a SBAR child is a tree instance and labeled WHNP
                                    found_whnp = True  # Assign to found_whnp (Identify the presence of WHNP)
                                elif isinstance(sbar_child,
                                                Tree) and sbar_child.label() == 'S' and found_whnp:  # If found SBAR child labeled S after found_whnp is True

                                    # CHECK FOR and OR
                                    has_np = any(child.label() == 'NP' for child in sbar_child if
                                                 isinstance(child, Tree))  # If S has a child NP, assign it to has_np
                                    has_vp = any(child.label() == 'VP' for child in sbar_child if
                                                 isinstance(child, Tree))  # If S has a child VP, assign it to has_vp
                                    ...
                                    if has_np and has_vp:  # if there are NP and VP as S's children indicating OR (I think the order of NP and VP doesn't matter here)
                                        for np_child in sbar_child:  # this naming 'sbar_child' is very confusing but actually the code is correct -___-
                                            if isinstance(np_child,
                                                          Tree) and np_child.label() == 'NP':  # if found a tree instance labeled NP under S
                                                has_prp = any(grandchild.label() == 'PRP' for grandchild in np_child if
                                                              isinstance(grandchild,
                                                                         Tree))  # if found a tree instance labeled PRP under NP, assign to has_prp
                                                if has_prp:  # if has_prp True
                                                    add_structure_counts(
                                                        "or_no_intv")  # Add 'or_no_intv' to structure_counts
                                                    return True  # Stop further processing since the structure has been identified
        ...
        if not structure_counts:
            structure_counts['Other_Structure'] = 1

    return structure_counts  # Return structure_counts for the main function so the output would be the sentence structure and the number of its occurrences
